...
namespace DrugPolicyFix
{
	...
	public class DrugPolicyUtility
	{
		...
		{
			if (((Drug != null) ? Drug.comps : null) != null)
			{
				List<CompProperties> comps = Drug.comps;
				if (comps.Count > 0)
				{
					using (List<CompProperties>.Enumerator enumerator = comps.GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							CompProperties_Drug compProperties_Drug;
							if ((compProperties_Drug = (enumerator.Current as CompProperties_Drug)) != null && compProperties_Drug.chemical == ChemicalDefOf.Alcohol)
							{
								return true;
							}
						}
			...
			if (((Drug != null) ? Drug.comps : null) != null)
			{
				...
				if (comps.Count > 0)
				{
					using (List<CompProperties>.Enumerator enumerator = comps.GetEnumerator())
					{
						...
						{
							CompProperties_Drug compProperties_Drug;
							if ((compProperties_Drug = (enumerator.Current as CompProperties_Drug)) != null && compProperties_Drug.chemical == DefDatabase<ChemicalDef>.GetNamed("Smokeleaf", true))
							{
								return true;
							}
						}
